...
TwoWire::TwoWire(uint8_t mode)
{
	...
	recv.buffer = new uint8_t[TWOWIRE_BUFFER_SIZE];
	xmit.buffer = new uint8_t[TWOWIRE_BUFFER_SIZE];
...
{
	pXmit->wrPtr = 0;
	pXmit->numSpace = TWOWIRE_BUFFER_SIZE;
	pXmit->rdPtr = 0;
	pXmit->numData = 0;
}
...
{
	pRecv->wrPtr = 0;
	pRecv->numSpace = TWOWIRE_BUFFER_SIZE;
	pRecv->rdPtr = 0;
	pRecv->numData = 0;
}
...
int TwoWire::read(void)
{
	...
	if (pRecv->numData) {
		...
		pRecv->rdPtr = (pRecv->rdPtr + 1) % TWOWIRE_BUFFER_SIZE;
		pRecv->numData --;
		pRecv->numSpace ++;
		value = (int)data;
	}
	...
	if (regData & 0x01) {
		...
		for (i = 0; i < rx_num; i++) {
			if (pRecv->numSpace) {
				...
				pRecv->wrPtr = (pRecv->wrPtr + 1) % TWOWIRE_BUFFER_SIZE;
				pRecv->numData ++;
				pRecv->numSpace --;
			}
